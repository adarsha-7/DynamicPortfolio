...
const callback = (res) => {
    ...
    document.querySelector("#resume-btn").addEventListener("click", () => {
        ...
        document.querySelector("#submit-button").addEventListener("click", () => {
            ...
            if (!newLink) alert("Please enter the new resume link before submitting");
            else {
                axios
                    .put(`${baseURL}/api/edit/resume`, { newLink: newLink })
                    .then((res) => {
    ...
    document.querySelectorAll(".project-card").forEach((card) => {
        ...
        card.querySelector(".work-edit").addEventListener("click", (event) => {
            ...
            form.className =
                "work-edit-form bg-cards dark:bg-cards-dark text-gray-800 dark:text-gray-300 h-4/5 w-4/5 max-w-200 z-5 fixed top-1/2 left-1/2 rounded-lg shadow-shadow p-4 flex flex-col items-center gap-5 border-b-1 border-r-1 border-gray-300 dark:border-gray-700 transform -translate-x-1/2 -translate-y-1/2";
            form.innerHTML = `
            ...
            }" id="form-title" class="w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 focus:outline-none" placeholder="Enter title..." required/>
            
            <label class="w-full text-md font-medium">Description</label>
            <textarea id="form-description" class="w-full flex-1 resize-none px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 focus:outline-none" placeholder="Enter description..." required>${
                card.querySelector(".project-info").innerHTML
            }</textarea>
            
            <label class="w-full text-md font-medium">Position</label>
            <input type="number" value="${
                card.querySelector(".project-position").textContent
            }" id="form-position" class="w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 focus:outline-none" placeholder="Enter position (example: 1)" required/>
            ...
            form.querySelector("#form-submit-btn").addEventListener("click", () => {
                ...
                axios
                    .put(`${baseURL}/api/edit/update-work`, newData)
                    .then((res) => {
            ...
            form.querySelector("#remove-work-btn").addEventListener("click", () => {
                ...
                confirmation.className =
                    "z-10 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-cards dark:bg-cards-dark text-gray-800 dark:text-gray-300 border border-gray-300 dark:border-gray-700 rounded-lg shadow-shadow p-6 w-80 flex flex-col items-center gap-4";
                ...
                confirmation.querySelector("#confirm").addEventListener("click", () => {
                    axios
                        .delete(`${baseURL}/api/edit/delete-work`, {
                            data: { title: card.id },
                        })
                        .then((res) => {
    ...
    document.querySelector("#add-project").addEventListener("click", () => {
        ...
        addForm.className =
            "add-form bg-cards dark:bg-cards-dark text-gray-800 dark:text-gray-300 h-4/5 w-4/5 max-w-200 z-5 fixed top-1/2 left-1/2 rounded-lg shadow-shadow p-4 flex flex-col items-center gap-3 border-b-1 border-r-1 border-gray-300 dark:border-gray-700 transform -translate-x-1/2 -translate-y-1/2";
        ...
        addForm.querySelector("#add-form-submit-btn").addEventListener("click", () => {
            ...
            frontend += addForm.querySelector('input[type="checkbox"][value="html"]').checked
                ? `<img src="/icons/html.png" alt="HTML logo">`
                : "";
            frontend += addForm.querySelector('input[type="checkbox"][value="css"]').checked
                ? `<img src="/icons/css.png" alt="CSS logo">`
                : "";
            frontend += addForm.querySelector('input[type="checkbox"][value="tailwind"]').checked
                ? `<img src="/icons/TailwindCSS.png" alt="TailwindCSS logo">`
                : "";
            ...
            backend += addForm.querySelector('input[type="checkbox"][value="node"]').checked
                ...
                : "";
            backend += addForm.querySelector('input[type="checkbox"][value="express"]').checked
                ? `<img src="/icons/expressjs-sm-gray.png" alt="Express.js logo">`
                : "";
            backend += addForm.querySelector('input[type="checkbox"][value="mongo"]').checked
                ? `<img src="/icons/mongodb-sm.png" alt="MongoDB logo">`
                : "";
            backend += addForm.querySelector('input[type="checkbox"][value="postgresql"]').checked
                ? `<img src="/icons/postgresql.png" alt="PostgreSQL logo">`
                : "";
            backend += addForm.querySelector('input[type="checkbox"][value="jwt"]').checked
                ? `<img src="/icons/jwt-sm.png" alt="JWT logo">`
                : "";

            axios
                .get(`${baseURL}/api/edit/next-position`)
                .then((res) => {
                    const newProject = {
                    ...
                    axios
                        .post(`${baseURL}/api/edit/add-work`, newProject)
                        .then((res) => {
